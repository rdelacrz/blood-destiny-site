...
const toIso = (raw: unknown): string =>
  typeof raw === 'string' ? raw.replace(/^!Date:/, '') : '';

/** Epoch ms for sorting (0 when unparseable). */
const dateMs = (raw: unknown): number => {
...
const resolveCover = (raw?: string | null): string | undefined => {
  const v = (raw ?? '').trim();
...
export const toUpdate = (dto: UpdatePostDto): Update => {
 ...
 */
export const fetchUpdates = async (signal?: AbortSignal): Promise<Update[]> => {
