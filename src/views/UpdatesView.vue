...
/* =====================================================================
   Updates — devlog timeline wired to the live feed (useUpdates). Keeps the
...
.tl-item__dot {
  position: absolute; left: 11px; top: 4px; width: 16px; height: 16px; border-radius: 50%;
  background: var(--bd-black); border: 2px solid var(--bd-crimson);
  box-shadow: 0 0 14px rgba(200,16,46,0.6); z-index: 1;
}
...
.tl-item__author { font-family: var(--f-ui); font-size: 0.62rem; letter-spacing: 0.18em; text-transform: uppercase; color: var(--bd-bone-mute); margin-bottom: 0.6rem; }
.tl-item__body { color: var(--bd-bone-dim); font-size: 0.95rem; }
.tl-item__body :deep(p) { margin: 0 0 0.7rem; }
.tl-item__body :deep(p:last-child) { margin-bottom: 0; }
.tl-item__body :deep(a) { color: var(--bd-crimson-hi); text-decoration: underline; text-underline-offset: 2px; transition: color 0.25s ease; }
.tl-item__body :deep(a:hover) { color: var(--bd-bone); }

/* =========================================================
   ASYNC STATES — error / empty panels + loading skeleton
   ========================================================= */
.state-panel { max-width: 600px; margin: 1rem auto 0; padding: 2.4rem 2rem; text-align: center; }
.state-panel__title { font-family: var(--f-display); font-size: 1.7rem; margin-bottom: 0.6rem; }
.state-panel__body { color: var(--bd-bone-dim); margin-bottom: 1.4rem; word-break: break-word; }
...
.skeleton--date { width: 130px; height: 12px; }
.skeleton--cover { width: 100%; height: 200px; border-radius: var(--rad); margin-bottom: 1rem; }
.skeleton--title { width: 70%; height: 22px; margin-bottom: 0.9rem; }
.skeleton--line { width: 100%; height: 12px; margin-bottom: 0.55rem; }
.skeleton--line.short { width: 55%; }

/* =========================================================
   LOAD MORE
   ========================================================= */
.tl-more { display: flex; justify-content: center; margin-top: 0.6rem; }
.tl-more__count { font-size: 0.66rem; letter-spacing: 0.12em; opacity: 0.65; }

/* =========================================================
   DESKTOP — centered zig-zag rail
   ========================================================= */
@media (min-width: 760px) {
  .timeline__line { left: calc(50% - 1px); }
  .tl-item { width: 50%; padding-left: 0; padding-right: 3rem; text-align: right; }
  .tl-item__dot { left: auto; right: -8px; }
  .tl-item__meta { justify-content: flex-end; }
  .tl-item.is-right { margin-left: 50%; padding-right: 0; padding-left: 3rem; text-align: left; }
  .tl-item.is-right .tl-item__dot { right: auto; left: -8px; }
  .tl-item.is-right .tl-item__meta { justify-content: flex-start; }
}
...
@media (prefers-reduced-motion: reduce) {
  .tl-item__card { transition: none; }
  .tl-item__card:hover { transform: none; }
}
</style>
