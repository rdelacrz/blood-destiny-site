...
/**
 * @fileoverview Loader — CSS-driven entrance/exit overlay shown until the app is ready.
...
.loader__logo {
  ...
  filter: drop-shadow(0 0 28px rgba(200,16,46,0.5));
...
.loader__line {
  width: 0;
  height: 1px;
  background: linear-gradient(90deg, transparent, var(--bd-crimson), transparent);
  transition: width .8s var(--ease-out) .5s;
}
...
.loader.is-anim .loader__logo {
  opacity: 1;
  transform: none;
}

.loader.is-anim .loader__line {
  width: 220px;
}

.loader.is-anim .loader__label {
  opacity: 1;
}
